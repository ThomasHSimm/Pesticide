...
def main(data_file = './data/combined_df.csv'):
    ...
    for postcode_col in postcode_cols_in_df:
        print(postcode_col)
        postcode_df = addto_pcode_mapper(df, shorter_pc_list,
...
def get_postcode_list(postcodes_df, usecols=['Postcode','mapArea'],
                    reduce_p_max=4
                    ):
    """
...
def addto_pcode_mapper(df, shorter_pc_list,
    ...
    df_pc['mapArea'] = _create_area_col(df_pc, postcodes_to_area_df[['Postcode','mapArea']], 'Postcode')
